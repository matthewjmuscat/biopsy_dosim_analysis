    ...
    *,
    ...
    with _font_rc(export_config):
        ...
        row_specs = [
            ...
            ("Dose grad (Gy/mm)", r"Dose-gradient magnitude $G_b(z)$ (Gy mm$^{-1}$)"),
        ]
        ...
        fig.text(0.015, 0.955, "a)", ha="left", va="top", fontsize=export_config.title_fontsize + 2)
        fig.text(0.015, 0.49, "b)", ha="left", va="top", fontsize=export_config.title_fontsize + 2)
        if legend_handles and legend_labels:
            fig.legend(
                legend_handles,
                legend_labels,
        ...
        fig.subplots_adjust(top=0.84, bottom=0.14, hspace=0.42, wspace=0.18)
        out_paths = _save_figure_multi(fig, save_dir, file_stem, export_config)
        plt.close(fig)
        return out_paths
...
def _draw_dvh_axis(
    ...
    *,
    ...
    _add_outside_annotation_box(ax, _build_trial_annotation_lines(trial_curves), export_config, y=1.115)
...
def plot_exemplar_cumulative_dvh_pair(
    ...
    *,
    ...
    with _font_rc(export_config):
        fig, axes = plt.subplots(1, len(biopsies), figsize=(6.8 * len(biopsies), 5.35), dpi=export_config.dpi, sharey=True)
        if len(biopsies) == 1:
            axes = [axes]
        ...
        fig.subplots_adjust(top=0.77, bottom=0.15, wspace=0.18)
        out_paths = _save_figure_multi(fig, save_dir, file_stem, export_config)
        plt.close(fig)
        return out_paths
    ...
    **kwargs: Any,
    ...
    with _font_rc(export_config):
        ...
        if show_markers:
            ...
            biopsy_to_marker = {label: marker_cycle[i % len(marker_cycle)] for i, label in enumerate(biopsy_labels)}
        ...
        if include_abs:
            y_label = rf"$\Delta {delta_type_text}_{{b,v}}^{{j}}$, $|\Delta {delta_type_text}_{{b,v}}^{{j}}|$ ({unit_text})"
        else:
            y_label = rf"$\Delta {delta_type_text}_{{b,v}}^{{j}}$ ({unit_text})"
        x_label = "Voxel index" if x_axis == "Voxel index" else r"Axial position along biopsy $z$ (mm)"
        ...
        _apply_publication_axis_style(ax, export_config, show_minor_x=False)
        if y_tick_decimals is not None:
            ax.yaxis.set_major_formatter(StrMethodFormatter(f"{{x:.{int(y_tick_decimals)}f}}"))
            ax.get_yaxis().get_offset_text().set_visible(False)

        handles = [Line2D([0], [0], color=color_map[j], lw=linewidth_signed, label=_latex_j(j)) for j in order_kinds]
        handles.append(Line2D([0], [0], color="black", lw=linewidth_signed, linestyle=_dash_style(linestyle_signed), label=rf"$\Delta {delta_type_text}$"))
        if include_abs:
            handles.append(Line2D([0], [0], color="black", lw=linewidth_abs, linestyle=_dash_style(linestyle_absolute), label=rf"$|\Delta {delta_type_text}|$"))
        if show_markers:
            biopsy_labels = list(dict.fromkeys(tidy["Biopsy"]))
            marker_cycle = ["o", "s", "^", "D", "P", "X", "v", ">", "<"]
            for idx, label in enumerate(biopsy_labels):
                handles.append(
                    Line2D(
                        [0],
                        [0],
        ...
        return _save_figure_multi(fig, save_dir, fig_name, export_config)
    ...
    **kwargs: Any,
    ...
    for pair in biopsies:
        ...
        for x_val in x_vals_sorted:
            ...
            signed_arrays.append(pd.to_numeric(group[signed_key], errors="coerce").dropna().to_numpy())
            abs_arrays.append(pd.to_numeric(group[abs_key], errors="coerce").dropna().to_numpy())
    ...
    with _font_rc(export_config):
        ...
        for idx, pair in enumerate(biopsies):
            ...
            pos_signed = lane_centers + biopsy_offsets[idx] - within_pair_offset
            pos_abs = lane_centers + biopsy_offsets[idx] + within_pair_offset
            signed_groups = per_signed[pair]
            abs_groups = per_abs[pair]
        ...
        if x_axis == "Voxel index":
            ...
            ax.set_xlabel("Voxel index", fontsize=export_config.axes_label_fontsize)
        else:
            ax.set_xticklabels([f"{v:g}" for v in x_vals_sorted])
            ax.set_xlabel(r"Axial position along biopsy $z$ (mm)", fontsize=export_config.axes_label_fontsize)
        ax.set_ylabel(_ylabel(), fontsize=export_config.axes_label_fontsize)
        ax.axhline(0.0, linestyle=":", linewidth=1.0, alpha=0.85, color="0.25")
        _apply_publication_axis_style(ax, export_config, show_minor_x=False)
        if y_tick_decimals is not None:
            ax.yaxis.set_major_formatter(StrMethodFormatter(f"{{x:.{int(y_tick_decimals)}f}}"))
        ...
        handles = [
            Patch(
                ...
                alpha=0.33,
                label=_biopsy_display_label(biopsies[i], biopsy_label_map),
            )
        ...
        fig.subplots_adjust(top=0.80, bottom=0.15)
        return _save_figure_multi(fig, output_dir, plot_name_base, export_config)
...
def plot_exemplar_length_scale_boxes(
    ...
    **kwargs: Any,
    ...
    with _font_rc(export_config):
        ...
        ax.set_xlabel(r"$\ell_k$ (mm)", fontsize=export_config.axes_label_fontsize)
        ax.set_ylabel(y_label, fontsize=export_config.axes_label_fontsize)
        if y_trim:
            y_upper = max(work.groupby(x_col)[y_col].quantile(0.95).max(), work[y_col].max() * 0.9)
            y_lower = y_min_fixed if y_min_fixed is not None else max(0.0, work[y_col].quantile(0.05))
            ax.set_ylim(y_lower, y_upper * 1.05)
        _apply_publication_axis_style(ax, export_config, show_minor_x=False)
        if y_tick_decimals is not None:
            ax.yaxis.set_major_formatter(StrMethodFormatter(f"{{x:.{int(y_tick_decimals)}f}}"))
            ax.get_yaxis().get_offset_text().set_visible(False)
        legend_handles = [
            Patch(facecolor=BIOPSY_PALETTE[i % len(BIOPSY_PALETTE)], edgecolor=BIOPSY_PALETTE[i % len(BIOPSY_PALETTE)], alpha=0.33, label=label)
        ...
        fig.subplots_adjust(top=0.80, bottom=0.15)
        return _save_figure_multi(fig, save_dir, file_name, export_config)
...
def plot_exemplar_voxel_pair_heatmap(
    ...
    **kwargs: Any,
    ...
    cbar_label_fontsize = kwargs.pop("cbar_label_fontsize", export_config.legend_fontsize)
    ...
    cbar_pad = kwargs.pop("cbar_pad", 0.6)
    ...
    for key in all_keys:
        ...
        with _font_rc(export_config):
            ...
            if show_title:
                ax.set_title(str(save_name_base), fontsize=export_config.title_fontsize)
            ...
            if color_bar_positions == "left_right":
                cax_lower = divider.append_axes("left", size="4%", pad=cbar_pad)
                cax_upper = divider.append_axes("right", size="4%", pad=cbar_pad)
                ...
                cbar_lower = plt.colorbar(mpl.cm.ScalarMappable(norm=mpl.colors.Normalize(*lower_limits), cmap=cmap), cax=cax_lower, orientation="horizontal")
                cbar_upper = plt.colorbar(mpl.cm.ScalarMappable(norm=mpl.colors.Normalize(*upper_limits), cmap=cmap), cax=cax_upper, orientation="horizontal")
            ...
            file_stem = f"{save_name_base}_{_sanitize_file_label(label)}"
...
def plot_exemplar_ridgeline_pair(
    ...
    *,
    ...
    biopsy_label_map: Mapping[tuple[str, int], str] | None = None,
    x_label: str = "Dose (Gy)",
    y_label: str = r"Axial position along biopsy $z$ (mm)",
) -> list[Path]:
    ...
    with _font_rc(export_config):
        ...
        for idx, (ax, pair) in enumerate(zip(axes, biopsies)):
            ...
            q05_col = _mi(stats_sub, "Dose (Gy)", "quantile_05")
            q95_col = _mi(stats_sub, "Dose (Gy)", "quantile_95")
            x_min = float(pd.to_numeric(stats_sub[q05_col], errors="coerce").min())
            x_max = float(pd.to_numeric(stats_sub[q95_col], errors="coerce").max())
            ...
            ax.set_yticklabels([f"{val:.1f}" for val in z_tick_vals], fontsize=export_config.tick_label_fontsize - 1)
            ax.set_xlabel(x_label, fontsize=export_config.axes_label_fontsize)
        ...
        fig.subplots_adjust(top=0.88, bottom=0.10, wspace=0.18)
