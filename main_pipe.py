...
def main():
    ...
    #### Gate variables
    qa_path1_gate = True
    delta_metrics_gate = False 
    cohort_plots_gate = False


    ### gates for plots 
    cohort_effect_sizes_heatmaps_plots = False
    cohort_deltas_boxplots = False
    cohort_dvh_boxplots = True
    cohort_length_scales_boxplots_with_curves = True
    cohort_global_scores_boxplots = False
    cohort_ridgeline_dose_plot = False
...
MultiIndex([(                   'Patient ID',      ''),
    ...
    load_old_cumulative_DVH_data = False
    if load_old_cumulative_DVH_data:
    ...
    # -------------------------------------------------------------
    # OUTPUT PATHS SETUP
    # Creates: output_dir and figure subdirs used by downstream plots
    # -------------------------------------------------------------
    ...
    # ------------------------------------------------------------------
    # COHORT HISTOGRAMS (raw voxel dose & gradient)
    # Dependencies: all_voxel_wise_dose_df (post filtering), cohort_output_figures_dir
    # Produces: histogram_fit_all_voxels_dose.{pdf,svg}, histogram_fit_all_voxels_dose_gradient.{pdf,svg}
    
    
    if cohort_plots_gate:

        helper_funcs.print_banner(
            title="COHORT HISTOGRAMS",
            subtitle="raw voxel dose & grad (filtered)",
            items={
                "n_rows": len(all_voxel_wise_dose_df),
                "dose_col": "Dose (Gy)",
                "grad_col": "Dose grad (Gy/mm)",
            },
            width=78,
        )

        dists_to_try = ["lognorm"]  # best fit for most of the data
        xrange_dose = (0, 60)
        xrange_dose_grad = (0, 10)
    ...
    # -------------------------------------------------------------
    # DVH METRICS PER TRIAL (MC)
    # Inputs: all_voxel_wise_dose_df (filtered), output_dir
    # Outputs: dvh_metrics_dir/Cohort: DVH metrics per trial.csv
    # -------------------------------------------------------------
    helper_funcs.print_banner(
        title="DVH METRICS PER TRIAL",
        subtitle="per-MC trial DVH stats",
        items={"n_rows": len(all_voxel_wise_dose_df)},
        width=78,
    )
    print("Computing DVH metrics per trial...")
    ...
    # -------------------------------------------------------------
    # DVH METRICS STATS PER BIOPSY (COHORT SUMMARY)
    # Inputs: calculated_dvh_metrics_per_trial_df
    # Outputs: dvh_metrics_dir/Cohort_DVH_metrics_stats_per_biopsy.csv
    # -------------------------------------------------------------
    helper_funcs.print_banner(
        title="DVH METRICS STATS PER BIOPSY",
        subtitle="cohort summary from per-trial DVHs",
        items={"rows": len(calculated_dvh_metrics_per_trial_df)},
        width=78,
    )
    print("Computing DVH metrics statistics per biopsy...")
    ...
    # -------------------------------------------------------------
    # PATH 1 QA: thresholds → logit models → design matrices → figs
    # Inputs: calculated_dvh_metrics_per_trial_df (filtered)
    # Outputs: qa_dir/{1_core..6_secondary_scan} CSVs and figures
    # -------------------------------------------------------------
    ...
    if qa_path1_gate:
        helper_funcs.print_banner(
            title="PATH 1 QA",
            subtitle="thresholds · logit models · design · figs",
            items={
                "rows_dvh_trials": len(calculated_dvh_metrics_per_trial_df),
                "qa_dir": str(qa_dir),
            },
            width=78,
        )
        ...
        remove_outlier_manually = False
        if remove_outlier_manually: # remove outlier manually
        ...
        print("Skipping PATH 1 QA analysis")
    ...
    # -------------------------------------------------------------
    # VOXEL-WISE NOMINAL vs MC DELTAS (TRIAL-LEVEL)
    # Inputs: all_voxel_wise_dose_df
    # Outputs: voxel_wise_nominal_analysis_dir/*.csv
    # -------------------------------------------------------------
    
    if delta_metrics_gate:
    
        helper_funcs.print_banner(
            title="VOXEL DELTAS (MC vs nominal)",
            subtitle="trial-level deltas + CLES summaries",
            items={"n_rows": len(all_voxel_wise_dose_df)},
            width=78,
        )
    ...
    Index([                                               'Patient ID',
        ...
        # ------------------------------------------------------------------
        # Sextant-based summaries of dose, gradient, and deltas
        # ------------------------------------------------------------------
    ...
    # END OF DELTAS VS GRADIENT ANALYSIS
    ...
    # -------------------------------------------------------------
    # GLOBAL DOSIMETRY (BY BIOPSY)
    # Inputs: cohort_global_dosimetry_df
    # Outputs: global_dosimetry_dir/cohort_global_dosimetry_by_biopsy.csv + stats
    # -------------------------------------------------------------
    helper_funcs.print_banner(
        title="GLOBAL DOSIMETRY (BIOPSY)",
        subtitle="summary stats per biopsy",
        items={"n_rows": len(cohort_global_dosimetry_df)},
        width=78,
    )
    ...
    # -------------------------------------------------------------
    # GLOBAL DOSIMETRY (VOXEL)
    # Inputs: cohort_global_dosimetry_by_voxel_df
    # Outputs: global_dosimetry_by_voxel_dir/global_dosimetry_by_voxel_statistics_all_patients.csv
    # -------------------------------------------------------------
    helper_funcs.print_banner(
        title="GLOBAL DOSIMETRY (VOXEL)",
        subtitle="summary stats per voxel",
        items={"n_rows": len(cohort_global_dosimetry_by_voxel_df)},
        width=78,
    )
    ...
    # -------------------------------------------------------------
    # DVH METRICS ANALYSIS (PIVOTED METRICS)
    # Inputs: cohort_global_dosimetry_dvh_metrics_df
    # Outputs: dvh_metrics_dir/dvh_metrics_statistics_all_patients.csv
    # -------------------------------------------------------------
    helper_funcs.print_banner(
        title="DVH METRICS ANALYSIS",
        subtitle="pivoted metric summaries",
        items={"n_rows": len(cohort_global_dosimetry_dvh_metrics_df)},
        width=78,
    )
    ...
    # -------------------------------------------------------------
    # EFFECT SIZES ANALYSIS (Cohen/Hedges/mean diff)
    # Inputs: all_voxel_wise_dose_df
    # Outputs: effect_sizes_analysis/*.csv
    # -------------------------------------------------------------
    helper_funcs.print_banner(
        title="EFFECT SIZES",
        subtitle="Cohen/Hedges/mean diff per voxel",
        items={"n_rows": len(all_voxel_wise_dose_df)},
        width=78,
    )
    ...
    # -------------------------------------------------------------
    # DOSE DIFFERENCES ACROSS LENGTH SCALES
    # Inputs: all_voxel_wise_dose_df
    # Outputs: length_scales_dosimetry/*.csv
    # -------------------------------------------------------------
    helper_funcs.print_banner(
        title="LENGTH-SCALE DOSE DIFFERENCES",
        subtitle="pairwise voxel deltas across scales",
        items={"n_rows": len(all_voxel_wise_dose_df)},
        width=78,
    )
    ...
    if not cohort_deltas_boxplots:
        print("Skipping!")
    else:

        # 1. Dose delta boxplots
        print("Generating dose delta boxplots...")
        if delta_metrics_gate:
            _ = production_plots.plot_cohort_deltas_boxplot(
                nominal_deltas_df_with_abs,
                save_dir=cohort_output_figures_dir,
            ...
            # 2. Dose delta boxplots by voxel
            _ = production_plots.plot_cohort_deltas_boxplot_by_voxel(
                nominal_deltas_df_with_abs,
    ...
    if not cohort_effect_sizes_heatmaps_plots:
        print("Skipping!")
    else:
    ...
    if not cohort_length_scales_boxplots_with_curves:
        print("Skipping!")
    else:
    ...
    if not cohort_global_scores_boxplots:
        print("Skipping!")
    else:
    ...
    print("Generating cohort ridgeline dose plot...")
    if not cohort_ridgeline_dose_plot:
        print("Skipping!")
    else:
    ...
    if delta_metrics_gate:
        ...
        print("Generating individual patient dosimetry and dose gradient deltas plots...")
    ...
    if not load_old_cumulative_DVH_data:
        print("Computing cumulative DVH curves per MC trial...")
